...
ユーザー情報はCognitoをマスターとし、サインイン時に表示名およびメールアドレスをDynamoDBに更新登録する。本データは、`/user/{user_id}` 画面と、チャット・ドキュメント一覧の投稿者名を表示するためのキャッシュデータである。一覧では、表示中の投稿者のプロフィールをBatchGetItemでまとめて取得する。
